...
Python支持可选地显式声明返回类型(`通过"->" expression`语法)，如果不声明则语言会根据return语句后的表达式进行推导。如果没有`return`语句，或者`return`后什么都不写，则函数默认返回一个特殊的值：`None`。
...
> Python中的函数是一等公民，**可以赋值给变量，也可以作为函数的参数和返回值**（函数式编程中的高阶函数），你可以把它当成一个普通的变量来使用。
...
> Java,C#,Python的变量都是引用语义，变量名称只是绑定到内存中具体变量的一个标签，堆内存中不再使用的变量回收依赖于GC(Garbage Collector)。而像是C,C++则是标准的值语义，它们需要自己维护变量的生命期，对于需要引用传递的情况，则直接传递指针型。还有一些特立独行的，像是GO是值语义，但slice和map除外，Rust无需GC，因为它~~天生就贱~~，使用高贵的所有权。
...
很多语言都支持可变参数列表，但是手法上八仙过海、各显神通。Python的星号表达式的背后是对序列型的解包操作，比如你甚至可以写：

```python
l = [1,2,3]
print(add(0, *l))
print(add(0, *(1,2,3)))
print(add(0, *range(1,10)))
```
...
双星号表达式实际上是对`dict`的解包操作，我们甚至可以直接传入一个解包`dict`：

```python
# **解包一个临时dict对象
print(add(0, 1, 2, **{'a':3,'b':4}))
```

对于普通的关键字参数，也可以通过`dict`来传递：
